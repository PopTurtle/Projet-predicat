...
let intset_to_string (iset : IntSet.t) : string =
  if IntSet.is_empty iset
    then "∅"
    else "{" ^ (String.concat ", " (List.map string_of_int (IntSet.to_list iset))) ^ "}"
...
let diag_to_predicate_def (d : diagramme) : int option * predicate_def =
  ...
  let p, m = Diag.fold (fun ensset fillv acc -> (
    if fillv = Vide then acc else (
      Predicate_set.fold (fun str acc' -> ens_add_int str ((snd acc) + 1) acc') ensset (fst acc), (snd acc) + 1
    )
  )) d (Predicate_def.empty, -1)
  
  in (if m < 0 then None else Some m), p
